...
<c:import url="../layout/app.jsp">
    <c:param name="content">
        ...
        <c:choose>
            <c:when test="${reports.size()==0 || reports==null}">
                        <h2>日報は登録されていません。</h2>
                    </c:when>
            <c:otherwise>
                <h2>
                    <c:out value="${reports[0].employee.name}" />
                    の日報
                </h2>
                        ...
                        <c:forEach var="report" items="${reports}" varStatus="status">
                            ...
                            <tr class="row${status.count % 2}">
                                ...
                                <td class="report_date"><fmt:formatDate
                                        value='${reportDay}' pattern='yyyy-MM-dd' /></td>
                ...
                <div id="pagination">
                    ...
                    <c:forEach var="i" begin="1"
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a
                                    href="<c:url value='?action=${actRep}&command=${commFoledIdx}&id=${reports[0].employee.id }&page=${i}' />"><c:out
                                        value="${i}" /></a>&nbsp;
                    </c:otherwise>
                        </c:choose>
                    </c:forEach>
        ...
        <p>
            <a href="<c:url value='?action=${actFol}&command=${commIdx}' />">フォロー一覧に戻る</a>
        </p>
    </c:param>
</c:import>